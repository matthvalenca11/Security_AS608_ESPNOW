...
// Altere esse MAC para o do outro ESP32
uint8_t peerMac[] = { 0xA4, 0xE5, 0x7C, 0x47, 0x34, 0xF4 };  // <- Atualize para o MAC real do outro ESP

// Estrutura do pacote
typedef struct {
  uint16_t size;
  uint8_t data[768];  // Template completo (máx 768 bytes)
  uint8_t id;
...
WiFi.disconnect();         // força reassociação e liberação de qualquer conexão anterior
delay(100);
...
void enrollAndSaveNewTemplate() {
  ...
  if (result == ESP_OK) Serial.println("Template enviado via ESP-NOW!");
  else Serial.println("Erro ao enviar template via ESP-NOW.");
